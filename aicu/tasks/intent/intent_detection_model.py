...
from aicu.models.modeling_bert import BertClassificationModel
...
class IntentClsModel(object):
    def __init__(self, args, use_cuda=True):
        MODEL_CLASSES = {
            "bert": (BertConfig, BertClassificationModel, BertTokenizer),
        }
        self.args = args
    ...
    def load_and_cache_examples(self, examples, do_cache=True, multi_processing=True):
        ...
        cache_feature_file = os.path.join(self.args.cache_dir,
                                          'cached_{}_{}_{}_{}_{}'.format(
                                              mode, self.args.model_type, self.args.max_seq_len, self.num_labels, len(examples) 
                                          ))
    ...
    def train(self):
        ...
        # eval_examples = self.processor.get_dev_examples(data_dir)
        train_dataset = self.load_and_cache_examples(train_examples)
        # dev_dataset = self.load_and_cache_examples(eval_examples)
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                        ...
                        if args.local_rank == -1 and args.evaluate_during_training:
                            results, model_outputs = self.evaluate()
        ...
        torch.save(args, os.path.join(final_model_dir, 'training_args.bin'))
        logger.info(f'Saving final checkpoint to {final_model_dir}')
    ...
    def evaluate(self):
        model = self.model
        args = self.args
        eval_dataset = self.load_and_cache_examples(self.processor.get_dev_examples(args.data_dir))
    ...
    def _get_inputs_dict(self, batch, no_label=False):
        inputs = {
            "input_ids":      batch[0],
    ...
    def predict(self, to_predict):
        ...
        eval_dataset = self.load_and_cache_examples(eval_example, do_cache=False, multi_processing=False)
        eval_sampler = SequentialSampler(eval_dataset)
        eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size)
        preds = None
        output_label_ids = None
        ...
        for batch in tqdm(eval_dataloader, disable=True):
            batch = tuple(t.to(self.device) for t in batch)
            with torch.no_grad():
                inputs = self._get_inputs_dict(batch, no_label=True)
                outputs = model(**inputs)
                _, logits = outputs[:2]
            if preds is None:
                preds = logits.detach().cpu().numpy()
